...
export async function PATCH(req: Request) {
  ...
  if (data) return NextResponse.json({ campaign: data });
  let existing = supabaseAdmin.from("campaigns").select("status").eq("id", body.id);
  if (!p.isAdmin) existing = existing.eq("created_by", p.id);
  const { data: current } = await existing.maybeSingle();
  if (!current) return NextResponse.json({ error: "Campaign not found" }, { status: 404 });
  return NextResponse.json({
    error: current.status === "sent"
      ? "This campaign has already been sent. Use Reuse to create an editable copy."
      : `Campaign cannot be edited while its status is ${String(current.status).replaceAll("_", " ")}.`,
